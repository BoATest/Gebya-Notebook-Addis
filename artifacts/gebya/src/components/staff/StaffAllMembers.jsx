      ...
      <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--color-border-light)', background: 'var(--color-surface-alt)' }}>
        ...
        <div className="flex items-center gap-2 rounded-xl border px-3 py-2 bg-white" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <input
            type="text"
            value={searchQuery}
            onChange={e => { setSearchQuery(e.target.value); store.setSearchQuery(e.target.value); }}
            placeholder={t('Search staff...', 'ሰራተኞችን ፈልግ...')}
            className="flex-1 text-sm focus:outline-none bg-transparent"
            style={{ border: 'none', outline: 'none' }}
          />
          {searchQuery && (
            <button onClick={() => { setSearchQuery(''); store.setSearchQuery(''); }} className="text-gray-400 hover:text-gray-600 text-sm font-bold px-1">✕</button>
          )}
        </div>
      </div>
...
export default React.memo(StaffAllMembers);
