      ...
      {pendingDevices.length === 0 ? (
        ...
        <div className="space-y-2">
           {pendingDevices.map(d => (
             <div key={d.device_id} className="flex items-center justify-between rounded-xl border px-3 py-2.5"
               ...
               <div className="flex gap-1.5">
                 <button onClick={() => onApproveDevice?.(d.device_id)}
                   className="px-3 py-1.5 rounded-lg text-xs font-bold"
                   style={{ background: 'var(--color-primary)', color: 'var(--color-bg-white)' }}>
                   {t('Approve', 'አረጋግጥ')}
                 </button>
                 <button onClick={() => onRejectDevice?.(d.device_id)}
                   className="px-3 py-1.5 rounded-lg text-xs font-bold"
