...
function StaffTodayTeam({
  ...
  const {
    todayStaffSales: storeTodayStaffSales,
    todayStaffTransactions: storeTodayStaffTransactions,
    expandedStaffDrilldown: storeExpandedStaffDrilldown,
    setSettling,
    ...
    staffCollectNote,
    staffCollecting,
    setExpandedStaffDrilldown,
  } = useStaffStore(useShallow((s) => ({
    ...
    staffCollectCash: s.staffCollectCash,
    staffCollectTransfer: s.staffCollectTransfer,
    staffCollectNote: s.staffCollectNote,
    staffCollecting: s.staffCollecting,
    setExpandedStaffDrilldown: s.setExpandedStaffDrilldown,
  })));

  const memberRows = useMemo(() => {
    return activeStaff.map(m => {
      const isPendingDevice = (m.devices || []).some(d => d.device_status === 'pending' || d.pending);
      const isDrilled = storeExpandedStaffDrilldown === m.id;
      const sales = storeTodayStaffSales[m.id];
      const txns = storeTodayStaffTransactions[m.id] || [];
      const lastS = lastSettlementPerStaff[String(m.id)];
      const sDaysSince = lastS ? Math.floor((Date.now() - lastS.settled_at) / MS_PER_DAY) : null;
      ...
      return (
        <div key={m.id}>
          <div className="w-full px-4 py-3 flex items-center justify-between text-left">
            <button onClick={() => setExpandedStaffDrilldown(isDrilled ? null : m.id)}
  ...
  }, [activeStaff, storeExpandedStaffDrilldown, storeTodayStaffSales, storeTodayStaffTransactions, lastSettlementPerStaff, canManageTeam, setExpandedStaffDrilldown, setSettling, handleViewSettlement, t]);

  const inactiveRows = useMemo(() => {
    return inactiveStaff.map(m => (
  ...
  }, [inactiveStaff, canManageTeam, onReactivateStaffMember, t]);
...
export default React.memo(StaffTodayTeam);
