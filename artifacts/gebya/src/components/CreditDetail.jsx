      ...
      <div className="rounded-2xl p-5 shadow-sm border" style={{ background: '#fff', borderColor: '#f0e6d4' }}>
        ...
        <div className="space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-gray-800">{fmt(record.original_amount)} birr</span>
          </div>
          {record.paid_amount > 0 && (
            <div className="flex justify-between text-sm">
              <span style={{ color: '#6b7280' }}>Amount paid</span>
              <span className="font-semibold text-green-700">-{fmt(record.paid_amount)} birr</span>
            </div>
          )}
          <div className="border-t pt-3 flex justify-between" style={{ borderColor: '#f0e6d4' }}>
            ...
            <span className="font-black text-red-600 text-xl">{fmt(record.remaining_amount || 0)} birr</span>
          </div>
        </div>
      ...
      <div className="rounded-2xl p-4 border" style={{ background: '#fffbeb', borderColor: '#fde68a' }}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-gray-800 font-medium">{fmt(record.original_amount)} birr</span>
          </div>
          {record.paid_amount > 0 && (
            <div className="flex justify-between text-sm">
              <span style={{ color: '#6b7280' }}>Paid back</span>
              <span className="font-medium text-green-700">{fmt(record.paid_amount)} birr</span>
            </div>
          )}
          {record.remaining_amount > 0 && (
            <div className="flex justify-between text-sm">
              <span style={{ color: '#6b7280' }}>Remaining</span>
              <span className="font-medium text-red-600">{fmt(record.remaining_amount)} birr</span>
            </div>
          )}
        </div>
      </div>
